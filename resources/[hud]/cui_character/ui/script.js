...
function loadClothesTab(tab, clothesData, male) {
    ...
    topsoverlist.empty();
    topsoverlist.parents().eq(0).find('input').prop("max", clothesData.topsover.length)
    ...
    topsunderlist.parents().eq(0).find('input').prop("max", clothesData.topsunder.length)
    ...
    pantslist.parents().eq(0).find('input').prop("max", clothesData.pants.length)
    pantslist.empty();
    ...
    shoeslist.parents().eq(0).find('input').prop("max", clothesData.shoes.length)
    ...
    */
    ...
    maskslist.parents().eq(0).find('input').prop("max", clothesData.masks.length)
    let nomask = 0;
    maskslist.empty();
    ...
    neckarmslist.parents().eq(0).find('input').prop("max", clothesData.neckarms.length)
    neckarmslist.empty();
...
$('#main .menuclose').on('click', function(evt) {
    ...
    let message = 'Bạn có chắc là bạn muốn ' + action + ' những thay đổi của bạn và thoát?';
    let popupData = { 
        title: 'xác nhận', 
        message: message
    };
...
var intv;
var timeout;
$(document).on('mousedown', '.list .controls button', function(e){
    let self = this;
    timeout = setTimeout(function(){
        intv = setInterval(function(){
            let list = $(self).siblings('select').first();
            let numOpt = list.children('option').length
            let oldVal = list.find('option:selected');
            let newVal = null;
...
$(document).on('click', 'input:radio[name=sex]', function(evt) {
    if(radioChecked == false)
    {
        let popupData = {
            title: 'xác nhận', 
            message: 'Thay đổi giới tính của nhân vật của bạn sẽ đặt lại tất cả các tùy chỉnh hiện tại. Bạn có chắc chắn muốn làm điều này?'
        };
...
$(document).on('input', 'input[type=range].apparel', function(e){
    $(this).trigger('refresh');
    let value = $(this).val();
    let num = $(this).parents().eq(1).find('.num');
    num.text(value);
    let selected = $(this).parents().eq(0).find('option:selected');
    let select = $(this).parents().eq(1).find('select');
    let drwkey = select.data('drwkey');
    let drwval = selected.data('drawable');
    let texkey = select.data('texkey');
...
$(document).on('change', 'select.clotheslist.component', function(evt) {
    let slide = $(this).parents().eq(0).find('input');
    let num = $(this).parents().eq(1).find('.num');
    slide.val($(this).prop('selectedIndex'));
    num.text($(this).prop('selectedIndex'));
    let selected = $(this).find('option:selected');
