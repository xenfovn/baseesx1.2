...
if Config.UseDeferrals then
							...
							"body": [
								{
									...
									"items": [
										{
											"type": "Image",
											"url": "https://i.imgur.com/YybFZ6H.png?1",
											...
											"columns": [
												...
												},
												{
													...
													"items": [
														{
															"type": "Input.Text",
															"placeholder": "Chiều Cao (48-96 inches)",
															"id": "height",
															"maxLength": 2
														}
													]
								...
								{
									"type": "TextBlock",
									"text": "Bạn sẽ tự động đồng ý với tất cả các điều khoản và luật của server khi nhấn nút đăng ký",
									"isSubtle": true,
									"horizontalAlignment": "Center",
									"fontType": "Monospace",
									"weight": "Bolder",
	...
	AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
		if playerIdentity[xPlayer.identifier] then
			...
			xPlayer.setName(('%s %s'):format(currentIdentity.firstName, ""))
			xPlayer.set('firstName', currentIdentity.firstName)
			xPlayer.set('lastName', "")
...
elseif not Config.UseDeferrals then
	...
	RegisterNetEvent('esx:playerLoaded')
	AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
		print(json.encode(alreadyRegistered))
	...
	ESX.RegisterServerCallback('esx_identity:registerIdentity', function(source, cb, data)
		...
		if xPlayer then
			--if not alreadyRegistered[xPlayer.identifier] then
				--if checkNameFormat(data.firstname) and checkSexFormat(data.sex) and checkDOBFormat(data.dateofbirth) and checkHeightFormat(data.height) then
