...
function loadESXPlayer(identifier, playerId)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT accounts, job, job_grade, job2, job2_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', {
			...
			local job, grade, job2, grade2, jobObject, gradeObject, jobObject2, gradeObject2 = result[1].job, tostring(result[1].job_grade), result[1].job2, tostring(result[1].job2_grade)
			...
			if ESX.DoesJobExist(job2, grade2) then
				...
				print(('[es_extended] [^3WARNING^7] Ignoring invalid job2 for %s [job: %s, grade: %s]'):format(identifier, job2, grade2))
